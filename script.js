    ...
    try {
        ...
        if (!usuario) {
            const { data, error } = await supabase.from("usuarios").insert([
                { nome, contato, curso, codigo_grupo: codigoGrupo }
            ]);
    ...
    try {
        ...
        if (error && error.code === 'PGRST100') {
            const { data, error } = await supabase.from("grupos").insert([
                { codigo: codigoGrupo, membros: JSON.stringify([{ nome, contato, curso }]), mensagens: "[]" }
            ]);
            if (error) throw error;
            grupo = data[0];
        } else {
            let membros = grupo.membros ? JSON.parse(grupo.membros) : [];
            const jaEhMembro = membros.some(m => m.contato === contato);
            if (!jaEhMembro) {
                membros.push({ nome, contato, curso });
                const { error } = await supabase
                    .from("grupos")
                    .update({ membros: JSON.stringify(membros) })
                    .eq("codigo", codigoGrupo);
                if (error) throw error;
            }
        }
...
function carregarMembros(grupo) {
    const membros = grupo.membros ? JSON.parse(grupo.membros) : [];
    membrosDiv.innerHTML = membros.map(
        membro => `<p><strong>${membro.nome} (${membro.curso || "Sem curso"}):</strong> ${membro.contato}</p>`
    ).join("");
}
...
async function carregarMensagens(grupo) {
    try {
        ...
        mensagensDiv.innerHTML = mensagens.map(
            msg => `<p><strong>${msg.nome}:</strong> ${msg.texto}</p>`
        ).join("");
    } catch (err) {
...
async function enviarMensagem() {
    ...
    try {
        ...
        if (error) throw error;
        let membros = grupo.membros ? JSON.parse(grupo.membros) : [];
        let usuario = membros.find(membro => membro.contato === document.getElementById("contato").value);
