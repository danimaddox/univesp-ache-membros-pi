document.addEventListener('DOMContentLoaded', function () {
    window.addEventListener('load', function () {
        ...
        const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlhaHdwb2ppZ2d0aG1ieHVxYWt1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzgyNDk2OTgsImV4cCI6MjA1MzgyNTY5OH0.Ni9iO_jFXbzWTrxXxeudWJIyiJVO_LIjnhuDIehthCI"; // Sua chave Supabase
        ...
        async function entrarNoGrupo() {
            ...
            try {
                ...
                if (!grupo) {
                    // Tentativa de inserir o grupo
                    try {
                        const { data: novoGrupo, error: erroNovoGrupo } = await supabaseClient
                            .from("grupos")
                            .insert([{
                                codigo: codigoGrupo,
                        ...
                        if (erroNovoGrupo) {
                            console.error("Erro ao inserir grupo:", erroNovoGrupo.message);
                            // Tratar o erro de chave duplicada (409) especificamente
                            if (erroNovoGrupo.code === '23505') {
                                alert("Já existe um grupo com esse código. Tente novamente.");
                            } else {
                                alert("Erro ao criar grupo. Verifique o console para detalhes.");
                            }
                            throw erroNovoGrupo;
                        }
                        ...
                        console.error("Erro na inserção do grupo:", insertError);
                        throw insertError;
                    }
                } else {
                    ...
                    try {
                        ...
                        if (updateError) {
                            console.error("Erro ao adicionar membro:", updateError.message);
                            throw updateError;
                        }

                        grupo.membros.push(usuario.id);
                    } catch (updateError) {
                        console.error("Erro na atualização do grupo:", updateError);
                        throw updateError;
                    }
                }
